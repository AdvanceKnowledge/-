...
@interface ProvidentFundLoanView()<UITextFieldDelegate>
...
- (void)calculatedRealInterestRate{
    if (_borrowingNumber == 5) {
        ...
        if (_multiple ==  1) {
            ...
            _interestRateLab.text = [NSString stringWithFormat:@"%@%%",[NSString stringWithFormat:@"%.3f",0.275*_multiple]];
        }
    }else{
        _saveCurrentRate = @"3.25";
        if (_multiple ==  1) {
            _interestRateLab.text = [NSString stringWithFormat:@"%@%%",_saveCurrentRate];
        }else{
            _interestRateLab.text = [NSString stringWithFormat:@"%@%%",[NSString stringWithFormat:@"%.3f",0.325*_multiple]];
        }
    }
